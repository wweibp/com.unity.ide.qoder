...
{
    [InitializeOnLoad]
    public class QoderScriptEditor : IExternalCodeEditor
    {
        const string qoder_arguments = "qoder_arguments";
        const string qoder_userExtensions = "qoder_userExtensions";
        ...
        static readonly string[] k_SupportedFileNames = { "qoder.exe", "qoder.app", "qoder.cmd", "qoder", "Qoder.app", "Qoder.exe" };
        ...
        string Arguments
        {
            get => m_Arguments ?? (m_Arguments = EditorPrefs.GetString(qoder_arguments, DefaultArgument));
            set
            {
                m_Arguments = value;
                EditorPrefs.SetString(qoder_arguments, value);
            }
        }
        ...
        {
            get => EditorPrefs.GetString(qoder_userExtensions, string.Join(";", defaultExtensions));
            set => EditorPrefs.SetString(qoder_userExtensions, value);
        }
        ...
        public QoderScriptEditor(IDiscovery discovery, IGenerator projectGeneration)
        {
            m_Discoverability = discovery;
            m_ProjectGeneration = projectGeneration;
        }

        static QoderScriptEditor()
        {
            var editor = new QoderScriptEditor(new QoderDiscovery(), new ProjectGeneration(Directory.GetParent(Application.dataPath).FullName));
            CodeEditor.Register(editor);

            if (IsQoderInstallation(CodeEditor.CurrentEditorInstallation))
            {
                editor.CreateIfDoesntExist();
            }
        }

        static bool IsQoderInstallation(string path)
        {
